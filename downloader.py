...
DUAL HYBRID INGESTION & FALLBACK ARCHITECTURE:
    1. Primary (Server-Side):
       - Prioritizes `cookies.txt` (Netscape format with SID/HSID/3PSID) on server.
       - Runs optimal client mappings (cookies_default, cookies_android_vr, etc.).
    2. Fallback Mechanism 1 (Local Worker Relay):
       - If Render cloud datacenter IP is challenged by YouTube BotGuard, the server
         can delegate stream extraction to a registered Local PC Worker (Home residential IP)
         via `LOCAL_WORKER_URL` or `/api/downloader/register_worker`.
    3. Fallback Mechanism 2 (Client-Side Browser Bridge):
       - If all server-side modes fail, the visitor's browser automatically intercepts
         and resolves video metadata and streams directly from their own residential/mobile connection,
         uploading or streaming to the server for lossless FFmpeg processing.
    4. Diagnostics & Live Console:
       - Structured Why / Where / What / How diagnostic logging for 100% transparency.
       - Real-time Diagnostic Console in Web UI footer.

Standalone Worker Execution:
    python downloader.py --worker [PORT]   (default port: 5005)
"""
...
def _classify_error_detailed(e, mode, url):
    ...
    if "Sign in to confirm" in err_raw or "confirm you’re not a bot" in err_raw:
        ...
        why = (
            ...
            "Server requires active cookies.txt, Local Worker Relay, or Client-Side Bridge."
        )
        how = "Use full login cookies.txt or let Client-Side Browser Bridge resolve from visitor's connection."
    elif "403" in err_raw:
        ...
        how = "Using 'cookies_default' mode or Client-Side Bridge will generate valid, signed stream URLs."
        ...
        why = (
            "The requested player client does not provide progressive video streams for this specific video ID."
        )
...
def _resolve_via_local_worker(url, log_list=None):
    ...
    If configured, sends resolution request to a registered Local PC Worker (running on Home Wi-Fi)."""
    ...
    ts = _now_ts()
    msg = f"[{ts}] 🌉 [Local Worker Bridge] Delegating resolution to local worker: {worker_url}..."
    if log_list is not None:
        log_list.append(msg)
    try:
        print(f"[downloader:auth] {msg}", flush=True)
    except Exception:
        pass
        ...
        with urllib.request.urlopen(req, timeout=12) as resp:
            if resp.status == 200:
                ...
                if info:
                    ...
                    return info, None
                return None, res_json.get("error") or "Worker returned empty info"
    except Exception as e:
        err_msg = f"Local worker unreachable ({e})"
        if log_list is not None:
            log_list.append(f"[{_now_ts()}] ⚠️ [Local Worker Bridge] Failed: {err_msg}")
        return None, err_msg


def _resolve_via_public_api(url):
    """Fail-safe public instance resolver (Invidious / Piped) when all datacenter
    yt-dlp attempts are blocked by YouTube IP bot-guards."""
    ...
    for api_url in instances:
        try:
            ...
            continue
    return None, "Public resolver instances unreachable"
...
def _apply_auth_mode(opts, mode, client_potoken=None):
    ...
    if mode == "cookies_default":
        ...
        if cfile:
            opts["cookiefile"] = cfile
        ext_args = {"player_client": ["web", "web_safari"]}
        if client_potoken:
        ...
        ext_args = {"player_client": ["web_safari", "web_embedded", "web"]}
        if client_potoken:
            ext_args["po_token"] = [f"web+{client_potoken}"]
        opts["extractor_args"] = {"youtube": ext_args}
        if cfile:
...
def _extract_info_smart(url, extra_opts=None, download=False, client_potoken=None, log_list=None):
    ...
    # Fallback Mechanism 2: Public Invidious / Piped Mirror
    if not download:
        ts = _now_ts()
    ...
    if info is None:
        ...
        return jsonify({
            ...
            "troubleshoot": {
                ...
                "what": "Extraction failed across all server authentication attempts",
                "why": "Datacenter IP challenged by YouTube bot-guards. Client-Side Browser Bridge will auto-engage.",
                "how": "Client-Side Browser Bridge will resolve the stream from your clean residential connection."
            }
        }), 400
...
def downloader_page():
    """Standalone Downloader Web UI with dual hybrid ingestion and live diagnostic console."""
    return """<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .console-body {
      ...
      line-height: 1.7; max-height: 320px; overflow-y: auto; color: #94a3b8; background: #040711;
    }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p>Hybrid Ingestion Engine: Server yt-dlp + Local Worker Relay + Client Browser Bridge</p>
    ...
    <div class="card hidden" id="previewCard">
      <div class="card-header">
        ...
        <span id="resolverBadge" style="font-size:12px; padding:3px 8px; background:rgba(99,102,241,0.2); color:#c7d2fe; border-radius:6px;">Server-Resolved</span>
      </div>
    ...
    <!-- Live Diagnostic Console (Footer) -->
    <div class="console-card">
      <div class="console-header">
        ...
        <div class="console-tabs">
          ...
          <button class="console-tab" onclick="filterLogs('auth')">Auth & Relays</button>
      ...
      <div class="console-body" id="consoleBody">
        <div class="console-log log-info">[System Initialized] Hybrid Ingestion Ready. Dual fallbacks active.</div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    async function checkServerHealth(){
      try {
        ...
        if(data.found && data.has_full_youtube_auth){
          ...
          pill.innerHTML = `🍪 cookies.txt: Active (${data.size_str}) • Auth: Full`;
        } else if(data.found){
    ...
    function filterLogs(filter){
      ...
      body.innerHTML = '';
      allLogs.forEach(l => {
        if(filter === 'all' || (filter === 'err' && l.type === 'err') || (filter === 'auth' && (l.msg.includes('Step') || l.msg.includes('Bridge') || l.msg.includes('Worker') || l.msg.includes('Cookie')))){
          const el = document.createElement('div');
    ...
    async function fetchVideoInfo(){
      ...
      try {
        ...
        addConsoleLog(`❌ [Network Failure] Server connect error: ${e.message}`, 'err');
      }
      ...
      addConsoleLog('🔄 [Client-Side Bridge] Server Datacenter IP blocked by YouTube. Engaging Client-Side Ingestion from your clean connection...', 'warn');
      ...
      if(vid){
        ...
        if(clientResolved){
          ...
          addConsoleLog(`✅ [Client-Side Bridge] Successfully resolved "${clientResolved.title}" directly via your clean connection!`, 'ok');
          return;
        }
      }

      btn.disabled = false; btn.innerHTML = '<span>🔍 Fetch Details</span>';
      addConsoleLog(`❌ [Extraction Failed] ${data ? data.error : 'Could not resolve video'}`, 'err');
    }
    ...
    async function resolveViaClientBrowser(vid, origUrl){
      ...
      for(const ep of endpoints){
        try {
          addConsoleLog(`[Client-Bridge] Querying public API instance: ${ep}...`, 'info');
          const resp = await fetch(ep, {signal: AbortSignal.timeout(6000)});
          if(resp.ok){
    ...
    async function startDownload(formatId, rawStreamUrlEncoded){
      ...
      // If client bridge resolved with a direct playable stream URL
      if(rawUrl && rawUrl.startsWith('http')){
        addConsoleLog(`[Direct Stream] Downloading stream link via browser bridge...`, 'ok');
      ...
      // Otherwise execute Server Download Job
      addConsoleLog(`[Download Triggered] Initiating server download for format [${formatId}]...`, 'info');
