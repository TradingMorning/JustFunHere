...
_FETCH_COOKIE_BROWSERS = ["chrome", "edge", "firefox", "brave"] if os.name == "nt" else []
...
def _fetch_auth_attempts():
    ...
    cookies_file = (SRC_DIR.parent / "cookies.txt") if SRC_DIR else (BASE / "cookies.txt")
    if not cookies_file.exists():
        for _sec in (Path("/etc/secrets/cookies.txt"), BASE / "cookies.txt"):
            if _sec.exists():
                cookies_file = _sec
                break
