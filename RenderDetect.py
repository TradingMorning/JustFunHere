...
def _apply_auth_mode_to_opts(opts, mode):
    ...
    Dynamically preserves and attaches cookies to all requests on cloud/datacenter IPs."""
...
def _extract_info_with_auth(url, extra_opts=None, download=False, progress_hook=None):
    """Shared auth-fallback loop (web_safari_highres -> android_mobile -> tv_embedded -> ios_mobile -> cookies_file -> oauth2 -> default -> bypass -> browsers -> public API),
...
def _auth_attempts():
    ...
    then Android/TV/iOS clients, cookies/oauth, standard bypass, and local browsers."""
    attempts = []
