...
print("Cookies File Detected: ", COOKIES_FILE)
...
def _extract_info_with_auth(url, extra_opts=None, download=False, progress_hook=None):
    """Shared auth-fallback loop (default -> bypass -> cookies_file -> browsers),
    cached-mode-first. Uses web_embedded client for 100% reliable 1080p/4K download
    without 403 blocks."""
    ...
    last_err = None
    CLIENT_ARGS = {'youtube': {'player_client': ['web_embedded', 'android', 'web']}}
    for mode in _auth_attempts():
        ...
        try:
            ...
            if _RESOLVED_MODE.get("mode") == mode:
                _RESOLVED_MODE["mode"] = None
            # Agle auth-mode ko try karne se pehle is attempt ka adhura/corrupt
            # partial file hata do
...
def _auth_attempts():
    """Cached working mode first, then default/bypass (with web_embedded client),
    followed by cookies_file and browser fallback."""
    attempts = []
    if _RESOLVED_MODE["mode"] and _RESOLVED_MODE["mode"] != "bypass":
...
def cut_proxy(video_url, audio_url, start, end, out_path, video_headers=None, audio_headers=None):
    ...
    if video_url == audio_url:
        cmd = [FFMPEG, "-y"] + v_hdr + ["-ss", str(start), "-i", video_url, "-t", str(duration),
           ...
           "-c:a", "aac", "-b:a", "192k",
           "-threads", "2", "-movflags", "+faststart", str(out_path)]
    else:
...
def apply_silence_cut(path, keep_segments):
    ...
    cmd = [FFMPEG, "-y", "-i", str(path),
           ...
           "-c:a", "aac", "-b:a", "192k",
           "-threads", "2", "-movflags", "+faststart", str(tmp)]
    result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                             encoding="utf-8", errors="replace", **_no_console_kwargs())
...
def _hw_video_args(codec, kind, preset, crf):
    ...
    return ["-c:v", "libx264", "-preset", preset, "-crf", str(crf), "-pix_fmt", "yuv420p", "-threads", "2"]
...
def _encode_export_variant(src, w, h, s, title, trim_offset, src_duration, out_w, out_h, progress_callback=None):
    ...
    attempts.append(["-c:v", vcodec, "-preset", preset, "-crf", str(crf), "-pix_fmt", "yuv420p", "-threads", "2"])
...
async function fetchCutVideoDetails(){
  ...
  try{
    ...
    setFcInlineStatus('❌ Network error', true); setFcBtnLoading(false); return;
  }
  setFcBtnLoading(false);
  if(data.error){ setFcInlineStatus('❌ '+data.error, true); return; }
  setFcInlineStatus(`✅ Found "${data.title}" — ${(data.formats||[]).length} format(s) available. Pick one below, or just leave the Quality/Stream Type dropdowns as-is.`, false);
...
def main():
    ...
    init_render(standalone_app)
    port = int(os.environ.get("PORT", 5000))
    print(f"[RenderDetect Service] Running standalone on port {port}...")
    standalone_app.run(host="0.0.0.0", port=port, debug=False)
